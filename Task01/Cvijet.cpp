...
Cvijet::Cvijet(sf::RenderWindow& window) : window_(window), sunce_(65.f) {
    sunce_.setFillColor(sf::Color(255, 255, 0));
    sunce_.setPosition(sf::Vector2f(-100.f, 200.f));
}
...
void Cvijet::draw() {
    ...
    sunce_.setPosition(sf::Vector2f(sunce_x, sunce_y));
    window_.draw(sunce_);


    sf::RectangleShape stabljika(sf::Vector2f(13.f, 150.f));
    stabljika.setPosition(sf::Vector2f(360.f, 330.f));
    ...
    for (int i = 0; i < 8; i++) {
        latica.setPosition(sf::Vector2f(365.f, 240.f));
        latica.setRotation(sf::degrees(i * 45.f));
    ...
    tucak.setPosition(sf::Vector2f(345.f, 215.f));
